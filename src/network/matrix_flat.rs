...
impl Matrix{
    ...
    fn set_submatrix(&mut self, row_start: usize, col_start: usize, other: &Matrix) {
        for i in 0..other.rows {
            for j in 0..other.columns {
                self[row_start + i][col_start + j] = other[i][j];
            }
        }
    }
    ...
    fn new_empty(rows: usize, cols: usize) -> Matrix{
        Matrix{
            rows: rows,
            columns: cols,
            data: vec![vec![0.0; cols]; rows].into_iter().flatten().collect()
        }
    }
    ...
    pub fn new_random(rows: usize, cols: usize) -> Matrix{
        ...
        for row in 0..rows{
            for col in 0..cols{
                res[row][col] = rng.gen::<f32>() * 2.0 - 1.0;
            }
        }
        res
    }
    ...
    fn as_ndarray(&self) -> Array2<f32> {
        Array2::from_shape_fn((self.rows, self.columns), |(i, j)| self[i][j])
    }
    ...
    pub fn from_flat(rows: usize, cols: usize, data: Vec<f32>) -> Matrix{
        Matrix { rows, columns: cols, data }
    }
    pub fn map(&mut self, function: &dyn Fn(f32) -> f32) -> Matrix{
        Matrix::from_flat(self.rows, self.columns,(self.data).clone()
                     .into_iter()
